...
Last pass: `feat/adr-0027-prose-to-gated-emit` (ADR 0027: prose -> gated emit, goal+policies only).
...
| Semantic X6 gate commands | ADR 0024 + 0027 | real per-ADR gate commands (v1 echoes MET) |
...
- ADR 0027: prose -> gated emit (goal + policies only; no LLM in path)
- Manifesto Belief section
- `nlc-hub-audit` skill
- FINDINGS as sole live gap queue
- `nlc-pipeline-wire.py` sequencing X1->X2->X5->X3->X6
