# ADR 0012 — ADR precedence and rule adoption conflicts
...
Adopted rules (if/thens) can overlap on the same tagged primitive. Not every overlap is a conflict: `must encrypt` and `must not store plaintext` often compose. **UC14** is not “encrypt vs cannot store.”
...
2. Each adopted rule cites an **ADR id** and **precedence_tier** (and optional **precedence_rank** within tier).
...
5. **Adopt-time gate:** `python3 tools/check-rule-adoption.py <rules-file>` runs before rules are marked in force. Emit-time application is unchanged (ADR 0007).

## Consequences

- [`docs/USE-CASES.md`](../docs/USE-CASES.md) UC14 row uses a true conflict example, not encryption vs storage policy.
