...
FIXTURE = ROOT / "examples" / "retrying-nested-no-key"
...
def main() -> int:
    ...
    if not any(v[1] == "missing-idempotency-key" for v in violations):
        print("ASSERT:FAIL expected missing-idempotency-key")
        return 1
    print("ASSERT:PASS fixture still fails C15 (nested, no key)")
    return 0
