...
def rule_conflict_blockers(root: Path) -> list[str]:
    ...
    script = Path(__file__).resolve().parents[2] / "check-rule-adoption.py"
    proc = subprocess.run(
...
def rule_coverage_blockers(root: Path) -> list[str]:
    ...
    script = Path(__file__).resolve().parents[2] / "nlc_rule_coverage.py"
    proc = subprocess.run(
...
def verify_fast_blockers(root: Path) -> list[str]:
    ...
    reasons.extend(engine_runtime_tag_strict_blockers(root))
