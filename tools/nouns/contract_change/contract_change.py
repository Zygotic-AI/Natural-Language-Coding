...
def _fitness_blocker(root: Path, script: str, label: str, hub: Path) -> str | None:
    tools = hub / "tools"
    path = tools / script
    proc = subprocess.run(
...
def _has_changed_scope(root: Path, hub: Path) -> bool:
...
def _c10_script(root: Path, hub: Path) -> str:
    if _has_changed_scope(root, hub):
        return "fitness-c10-changed.py"
    return "fitness-c10.py"


def _c21_script(root: Path, hub: Path) -> str:
    if _has_changed_scope(root, hub):
        return "fitness-c21-changed.py"
    return "fitness-c21.py"


def contract_change_blockers(root: Path, hub: Path | None = None) -> list[str]:
    """C10 breaking schema + C21 impact list on non-hub product trees."""
    hub_path = (hub or HUB_ROOT).resolve()
    sys.path.insert(0, str(hub_path / "tools"))
    ...
    blockers: list[str] = []
    for script, label in (
        (_c10_script(root, hub_path), "C10"),
        (_c21_script(root, hub_path), "C21"),
    ):
        msg = _fitness_blocker(root, script, label, hub_path)
        if msg:
            blockers.append(msg)
