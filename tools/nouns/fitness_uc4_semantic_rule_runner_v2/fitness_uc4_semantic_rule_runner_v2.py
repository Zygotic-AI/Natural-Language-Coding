...
BOUNDARY_JSON = ROOT / "integrity" / "uc4-product-boundary.json"
...
def main() -> int:
    ...
    if not BOUNDARY_JSON.is_file():
        violations.append("missing integrity/uc4-product-boundary.json")
    else:
        data = json.loads(BOUNDARY_JSON.read_text(encoding="utf-8"))
        if data.get("remain_expansion"):
            violations.append("uc4-product-boundary remain_expansion must be empty")
    if STATUS.is_file():
        exp = (json.loads(STATUS.read_text(encoding="utf-8")).get("ucs") or {}).get(
            "UC4", {}
        ).get("expansion_only") or []
        if "semantic_rule_runner_adr_0007" in exp:
            violations.append("UC4 expansion_only must not list semantic_rule_runner_adr_0007")
    for script in (
        "assert-rule-semantic-pan-passes.py",
        "assert-rule-semantic-pan-fails.py",
        "assert-rule-runner-passes.py",
    ):
