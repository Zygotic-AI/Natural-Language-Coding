...
def _pending_adr_names(root: Path) -> list[str]:
    from nlc_dashboard import _list_proposed_adrs

    return _list_proposed_adrs(root / "adrs")
...
def _run_check_rules(root: Path, hub: Path) -> int:
    ...
    if not check.is_file():
        check = Path(__file__).resolve().parent / "check-rule-adoption.py"
...
def _maybe_regen_after_sync(root: Path, hub: Path) -> None:
    ...
    if not script.is_file():
        script = Path(__file__).resolve().parent / "nlc-delta-regen.py"
    subprocess.run(
        [
            sys.executable,
            str(script),
