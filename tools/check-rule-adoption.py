...
Input: rules JSON path; optional --overrides integrity/precedence-overrides.json
Output: ADOPTION:MET or ADOPTION:NOT_MET on stdout. Exit 0 / 1.
"""
...
def main() -> int:
    ...
    overrides_path = ROOT / "integrity" / "precedence-overrides.json"
    if "--overrides" in sys.argv:
        i = sys.argv.index("--overrides")
        if i + 1 < len(sys.argv):
            overrides_path = Path(sys.argv[i + 1]).resolve()

    prec = load_json(ROOT / "integrity" / "adr-precedence.json")
