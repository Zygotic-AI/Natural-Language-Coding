...
def rule_runner_blockers(root: Path) -> list[str]:
    ...
    if _hub_skip(root):
        return []
    from nlc_rule_runner import check_ir

    return check_ir(root)
...
def rule_apply_blockers(root: Path) -> list[str]:
    """UC5: compiler-owned markers cover adopted rules (apply check)."""
    if _hub_skip(root):
        return []
    adopted = root / "rules" / "adopted.json"
    if not adopted.is_file():
    ...
    tail = detail[-1] if detail else "rule coverage NOT_MET"
    return [f"UC5 rule apply: {tail}"]
