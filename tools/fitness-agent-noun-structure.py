#!/usr/bin/env python3
"""BBP fitness check: agent noun package structure validation (S1-S8, CS1-CS8).
...
Validates:
  S1/CS1: Agent noun has identity file with purpose and adjectives
...
def check_identity_and_adjectives(agent_md: str, agent_name: str) -> tuple[bool, str]:
    """S1/CS1: Check for identity (name, purpose) and adjectives section.

    Accepts ## Adjectives (current) or ## Invariants (legacy heading).
    """
    has_identity = "## identity" in agent_md or "**name:**" in agent_md
    has_purpose = "**purpose:**" in agent_md or "purpose:" in agent_md
    has_adjectives = (
        "## adjectives" in agent_md
        or "adjective" in agent_md
        or "## invariants" in agent_md
        or "invariant" in agent_md
    )

    if has_identity and has_purpose and has_adjectives:
        return True, "identity, purpose, adjectives present"
    missing = []
    if not has_identity:
        missing.append("identity")
    if not has_purpose:
        missing.append("purpose")
    if not has_adjectives:
...
def validate_agent_noun(agent_dir: Path) -> dict[str, tuple[bool, str]]:
    """Validate a single agent noun directory against S1-S8/CS1-CS8."""
    agent_md = read_file_lower(agent_dir / "AGENT.md")
    verbs_md = read_file_lower(agent_dir / "verbs.md")
    agent_name = agent_dir.name

    results = {}
    results["S1"] = check_identity_and_adjectives(agent_md, agent_name)
