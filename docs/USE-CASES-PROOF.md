...
| UC4 | `rule_ir_blockers`, `rule_runner_blockers` | `./nlc maintainer rule-runner`; `assert-rule-runner-fails` |
| UC5 | `rule_apply_blockers`, `rule_coverage_blockers` | `./nlc maintainer rule-emit`; `assert-adopter-verify-fast-green-passes` |
| UC9 | `goal_bindings_narrow_blockers` | `assert-goal-bindings-narrow-fails`, `assert-delta-regen-narrow-passes` |
| UC10 | `upstream_hand_patch_blockers` | `assert-upstream-hand-patch-fails` |
| UC13 | `durable_engine_blockers`, `durable_rule_blockers` | `assert-durable-engine-rule-fails` |
