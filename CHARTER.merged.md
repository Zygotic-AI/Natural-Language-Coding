...
- Does any sensitive adjective escape its consuming verb (stored, passed, returned)?
...
Code follows the ratified spec. This is PLANIT's generate step: the AI compiler emits BBA-shaped code for the bound statements only. Humans do not edit the output to "help."
...
- Every atomic statement's bindings are honored in the emitted code.

### Step 7 — Confirm

Run the [Confirmation checklist](#confirmation-checklist). Any fail is a failed change, not a note for later. This is PLANIT's prove step, executed as independent gates: one machine gate (field writes, taint lifetime, escape hatches) and one adversarial audit (statements done, bindings held, no second copy of an adjective).

Fail → RCA into the spec (interview, requirement, ADR, knowledge), then regenerate. Never patch the generated tree to make the audit quiet.
...
- Record any interview miss the RCA surfaced, so the log compounds into a better interview over time.
