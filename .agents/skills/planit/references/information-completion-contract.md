# Information-completion [contract](../../../../docs/TERMS.md#contract)

Normative rules for eliciting inputs until an agent can run a prompt or skill safely: resolution table, blocking vs optional facts, fast path, deliverable-write [gate](../../../../docs/TERMS.md#gate), and multi-stage [Handoff](../../../../docs/TERMS.md#handoff) validation.
...
**Profile:** This [contract](../../../../docs/TERMS.md#contract) is the **intake / deliverable-write [gate](../../../../docs/TERMS.md#gate) profile** ([`operation-verdict-standard.md`](operation-verdict-standard.md) §2.3). It inherits **[default-closed](../../../../docs/TERMS.md#default-closed)** posture and **testable true/false** criteria; do not weaken them here.

---

Published prompts and skilletted skills are **Markdown only**—agents follow this [contract](../../../../docs/TERMS.md#contract) by instruction; there is no runtime validator. Users may prepend answers or edit the paste block; the agent must still behave safely and avoid redundant questioning when the first message is already complete.
...
**Completion [rule](../../../../docs/TERMS.md#rule):** The agent treats the task as **information-complete** when Layer A is resolved **and** every **blocking** Layer B item is answered, explicitly **[waived](../../../../docs/TERMS.md#waived)** only where the prompt allows, or recorded as a labeled **`Assumption:`** (or equivalent) in the deliverable when the prompt permits assumptions instead of facts.

## Step 0 — Resolution table (before Execution)

**[AWL](../../../../docs/TERMS.md#awl) Phase 1:** This step satisfies **intake** in [`audited-work-loop-standard.md`](audited-work-loop-standard.md). For **T2+** work, use the extended intake table and blocking questions in [AWL](../../../../docs/TERMS.md#awl) **Appendix C** (tier compression for T0/T1 per AWL §5).
...
## Blocking [rule](../../../../docs/TERMS.md#rule): deliverable writes

Do **not** create or overwrite the prompt’s **primary deliverables** (files, mandated report sections, etc.) until **information-complete** per the completion [rule](../../../../docs/TERMS.md#rule).

**Read-only discovery** is allowed before completion when **Validation** requires it (e.g. locate `Dockerfile*`, confirm layout). Order steps so **invasive edits** never precede the [gate](../../../../docs/TERMS.md#gate); if a prompt’s Validation lists reads, perform those reads before claiming completion is impossible.
...
Where the prompt says a question may be **[waived](../../../../docs/TERMS.md#waived)** or a **default** applies, document the default in the resolution table and in the deliverable as an **`Assumption:`** when the prompt requires it. Do **not** invent unsafe defaults for compliance tier, scope, or security-sensitive choices unless the prompt explicitly allows assumption.
...
- **Stage prompts** that consume only prior output must **validate** required **[Handoff](../../../../docs/TERMS.md#handoff)** sections/headings. If missing or malformed, ask the user to paste the missing sections or **re-run** the prior stage—do **not** invent prior-stage content.
