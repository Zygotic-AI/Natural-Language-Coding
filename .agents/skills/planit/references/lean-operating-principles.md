...
## 1. [Jidoka](../../../../docs/TERMS.md#jidoka) — do not pass problems downstream

**Principle:** When you detect a [defect](../../../../docs/TERMS.md#defect), abnormality, or failed [gate](../../../../docs/TERMS.md#gate), **stop and address it** (or escalate with a clear blocker). Do not leave the problem for the next person, the next pipeline stage, or the next session.

| In manufacturing | In this vault |
| ---------------- | ------------- |
| Stop the line when [quality](../../../../docs/TERMS.md#quality) fails | Do not declare a task complete while CI, validators, or link checks fail |
...
When the user’s message (this turn or earlier in the same task thread) already ordered an [action](../../../../docs/TERMS.md#action) — including by clear implication — the agent **must execute** that [action](../../../../docs/TERMS.md#action). It is a **complete failure** to stop and ask whether to proceed with that same work.

| Already authorized (examples) | Forbidden agent response |
| ----------------------------- | ------------------------ |
| “[RCA](../../../../docs/TERMS.md#rca) and remediate” | “Want me to fix those now?” |
| “Did you fix them?” after an [RCA](../../../../docs/TERMS.md#rca) with open remediation | “Should I finish remediating?” |
...
| Correct andon | Stop after autonomous recovery is missing, unsafe, or already failed; or when standards reserve a human [gate](../../../../docs/TERMS.md#gate); or when a **new** blocking input is truly missing (information-completion) |
...
1. When designing a [gate](../../../../docs/TERMS.md#gate): if recovery is a known vault command (e.g. `bash tools/tree-sitter/install.sh`), **wire it to run autonomously**, then loop back to the same check.
...
4. After [RCA](../../../../docs/TERMS.md#rca) with a preventive [action](../../../../docs/TERMS.md#action), or after answering “no, not fixed yet,” **implement the remaining fixes in the same turn** — do not ask permission to continue authorized remediation.
5. Never end a turn with only “Want me to …?” when the user’s last clear intent was to have that work done.

**Anti-pattern (historical):** DSI Stage B asking the human y/n to install Tree-sitter when the skill could run `install.sh` and re-check. Asking “want me to remediate?” after the user ordered [RCA](../../../../docs/TERMS.md#rca) + remediate and then asked if fixes were done.
...
- [`root-cause-analysis-standard.md`](root-cause-analysis-standard.md) — One preventive change; prevention test; [RCA](../../../../docs/TERMS.md#rca) record
...
- [`audited-work-loop-standard.md`](audited-work-loop-standard.md) — Seven-phase work loop; [audit](../../../../docs/TERMS.md#audit) failure behavior; authorized-work exceptions
