# `/local-dsi-converge` audit template
...
**AWL mapping:** This template is a **Phase 6 execution-audit instance** of the org-wide [`audited-work-loop-standard.md`](audited-work-loop-standard.md) (adversarial execution audit with machine gates and spot checks). It is not a separate process from AWL.
...
| **Deliverable contract recorded** | `machine_converge_v1` / `consumer_dsi_full_v1` |
...
| **DSI-RECHECK.md action** | **None** / **Remove row** (remove only if lens A PASS **and** lens C PASS when strict caveats opted in) |
...
| **NOT 100% PASS** | Scope not fully met; ≠ machine **FAIL** unless lens A says **FAIL**. |
...
## Iron-clad process RCA (consolidated)

| Gap IDs | Process root | Iron-clad vault fix |
| ------- | ------------ | ------------------- |
| A′, F, E-NAR (lens B) | **P1** Deliverable contract split | `--deliverable-contract consumer_dsi_full_v1` + handoff gates |
| Q-ADV (lens C) | **P2** Audit vs stop predicate mismatch | Lens A only for RECHECK; lens C opt-in |
| Q-ADV (ETL shape) | **P3** Extract-or-slot advisories | Not a machine defect; no re-converge for ratios alone |

Per-stage RCAs must map to **P1**, **P2**, or **P3** — not ad-hoc “re-run converge” without contract change.
