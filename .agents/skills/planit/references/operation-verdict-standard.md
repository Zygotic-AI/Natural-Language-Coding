...
- A skill **Gate** or **Procedure** step
- A shell command or script invocation
- An AWL **phase** (especially adversarial plan audit and execution audit)
- A machine gate (`stop_check.py`, validator, preflight)
- A maintainer **handoff** or **audit** section
...
## 2. Gate design (produced artifacts)

Every **produced** runnable artifact — published prompt, skilletted skill, repo-local `.cursor/prompts/` or `.cursor/skills/` pack, pipeline stage, AWL phase with a stop, or machine preflight — **must** include at least one **Gate** (or equivalent gate subsection) that controls whether downstream work may proceed.

### 2.1 Default-closed posture

| Rule | Requirement |
| ---- | ----------- |
| **Default-closed** | Until the gate **PASS**es, the agent **must not** run deliverable writes, destructive commands, or dependent procedure steps. Unmet, waived-without-authority, or ambiguous criteria ⇒ **FAIL** or **BLOCKED** — not proceed-with-caveats. |
| **No soft open** | Forbidden: “mostly ready,” “good enough,” “continue unless blocked,” or implied PASS from partial satisfaction. |
| **Machine authoritative** | When a machine gate exists for a criterion, its result overrides chat self-attestation ([§3](#3-objective-success-criteria)). |
| **Jidoka** | A failing gate **stops** the workflow at source ([`lean-operating-principles.md`](lean-operating-principles.md) §1). |

### 2.2 Testable criteria (true/false)

Each **blocking** gate criterion must be **individually decidable** as **true** or **false** from cited evidence before the gate may PASS.
...
| **Declared before run** | Criteria appear in **Gate**, **Validation**, or an cited standard **before** execution — not invented after the fact. |

**Authoring shape (skills and prompts):** use a numbered gate list or a table:

| Criterion | Blocking? | Pass when (true if…) | Evidence |
| --------- | --------- | -------------------- | -------- |
| … | yes / no | objective condition | command, path, or acceptance record |

**Gate PASS:** every **blocking** row is **true**. Then emit the §4 verdict block with **`Result: PASS`**.

**Gate FAIL:** any **blocking** row is **false** ⇒ **`Result: FAIL`**; do not proceed.

**Gate BLOCKED:** a required test could not run (missing input, tool, or environment) ⇒ **`Result: BLOCKED`**.

### 2.3 Gate profiles (specializations)

These are **profiles** of §2 — do not redefine default-closed or testable criteria:
...
| **Plan / execution audit** | [`audited-work-loop-standard.md`](audited-work-loop-standard.md) §4 | Phases 4 and 6 adversarial audits |
| **Skilletted skill Gate section** | [`skillet-export.md`](skillet-export.md) §4 | Compiled `SKILL.md` body; compiler `GATE_SNIPPET` is the **intake** profile |
| **Machine gate** | Domain scripts (`stop_check.py`, `validate-skills.mjs`, …) | Exit code + structured output (`ok: true`) |

New profiles must cite this section and add only **domain-specific criteria rows**, not a weaker posture.

### 2.4 Anti-patterns

- Gate with no blocking criteria (everything de facto optional).
...
- Separate “validation” section that contradicts or bypasses **Gate**.
...
| **Declared upfront** | Skills and prompts state criteria in Gate, Procedure, or **Done signals** before execution. Ad-hoc operations in chat must state criteria immediately before running. |
| **Fail-closed** | If evidence is missing or ambiguous, verdict is **FAIL** or **BLOCKED** — not PASS with caveats. **INCONCLUSIVE** (could not tell whether the criterion holds) is **BLOCKED**, never PASS. |
| **Machine first** | When a machine gate exists, its result is authoritative over chat self-attestation. |
...
**Forbidden as sole evidence:** compile- or lint-only when the claimed defect was runtime; cached or derived freshness; a **delegate or subagent summary** without inspecting the produced files, diff, or machine output.
...
**Forbidden:** A closing paragraph that says "everything looks good" without **`Result: PASS`**; a PASS verdict followed only by "but note…" with no incidental section header; implying PASS because most steps succeeded while a required gate failed.
...
1. **Gate** — per [§2](#2-gate-design-produced-artifacts): default-closed; blocking criteria individually testable true/false.
2. **Done signals** — list objective PASS conditions (and what FAIL looks like).
3. **Per-step verdict** — multi-step procedures require a §4 verdict block after each step (or after each machine gate); handoff may summarize process verdict.
4. **`uses_standards`** — include `standards/operation-verdict-standard.md` when the skill produces gated or audited output.
5. **Scripts** — prefer exit codes; stdout may include `VERDICT: PASS | FAIL | BLOCKED`. Do **not** print PASS if the check did not run. Parsers treat a missing verdict the same as BLOCKED when the script is the machine gate.

AWL alignment: Phase 4 and Phase 6 audit memos satisfy this standard when they use **`Verdict: PASS | FAIL`** and per-register evidence ([`audited-work-loop-standard.md`](audited-work-loop-standard.md) §4).
...
2. Evaluate each **blocking** gate criterion true/false; do not proceed on any **false**, **untested**, or **INCONCLUSIVE** blocking row.
...
- [`information-completion-contract.md`](information-completion-contract.md) — intake gate profile
- [`dsi-converge-deliverable-contract.md`](dsi-converge-deliverable-contract.md) — Lens A **100% PASS** / **FAIL**
- [`vault-pull-request-review-standard.md`](vault-pull-request-review-standard.md) — merge gate verdicts
- [`skillet-export.md`](skillet-export.md) — skilletted skill Gate section
